#!/bin/sh 
export CLUSTER_ROLE_ARN="aws cloudformation --region us-east-2 list-exports  | jq '.Exports[] | select(.Name == "rolesinfra-ClusterRoleARN") | .Value' | tr -d \'\"" | sed -i "s|CLUSTER_ROLE_ARN|${CLUSTER_ROLE_AR}|" cluster.yaml
